...
class calculator {
	...
	fun run() {
		...
		try {
			...
			val v = e.variable
			if (v.first() != '/') println("Bad variable $v")
			else {
...
//	const val allowedChars = CharArray('
	...
	private fun pre_parse(line: String): Pair<MutableList<calculatorDefType>, MutableList<Operation.anyOperator>> {
		...
		val withoutDoublePlus = withoutSpace.replace("++", "+")
		val withoutDoubleMinus = withoutDoublePlus.replace("--", "+") // there is replacement for ^^9
		...
		for (ch in withoutDoubleMinus) {
			if (ch.isDigit() || ch == '.' || (ch == '-' && '-' !in lastNum?: "" && lastNum == null)) {
				if (lastNum == null) {
					lastNum = ""
				}
				lastNum += ch
