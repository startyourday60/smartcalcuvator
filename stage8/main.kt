...
typealias calculatorDefType = java.math.BigInteger
const val calculatorDefTypeText = "java.math.BigInteger"
const val calculateDefValue = "0"
class Operation
{
    companion object {
...
//    data class l_operator(override val text: String, override val t: type) : operator, anyOperator(true) 
    ...
    data class variable(val name: String, var value: calculatorDefType = calculatorDefType(calculateDefValue)) : anyOperator(true)
}
...
class Parser
{
	...
	private fun calculateNums(w: MutableList<Pair<Operation.anyOperator, calculatorDefType>>): calculatorDefType?
	{
		...
		val wSize = w.size
		if (wSize == 0) return  calculatorDefType(calculateDefValue)
		val (fOp, fNum) = w[0]
